...
export class RevenuesService extends BaseListService<RevenueGridModel> {

  constructor(
    @Inject('API_BASE_URL') apiBaseUrl: string,
    http: HttpClient
  ) {
    super(apiBaseUrl, 'revenues', http);
  }

  deleteRevenue(id: number, action = 'delete'): Promise<any> {
  ...
  createElementCSV(elementData, action = 'AddCSV'): Promise<any> {
    return new Promise((resolve, reject) => {
      this.create(elementData, action)
        .subscribe(response => {
          // this.getElements();
          resolve(response);
        },
